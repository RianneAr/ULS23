...
def create_supervised_lesion_data(
    ...
    for archive, lesion_mask in fully_supervised_archives:
        ...
        lbl_path = os.path.join(archives_folder, archive, "labels_baseline_predicted_and_filtered")

        data = []
        #checkls = list(set([x.split('_lesion_')[0]+ '.nii.gz' for x in os.listdir('/nnunet_data/nnUNet_raw/DeepLesion_preprocessed/imagesTr')]))
        print('test',len([fn for fn in os.listdir(lbl_path) if fn[0] != "."]))
        for file in [fn for fn in os.listdir(lbl_path) if fn[0] != "."]:
            data.append(
                {
...
def main():
    archives_folder = "/nnunet_data_og"
    output_path = "/nnunet_data/nnUNet_raw"
